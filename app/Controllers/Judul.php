...
class Judul extends BaseController
{
    ...
    public function simpan()
    {
        ...
        [$similarityScore, $similarityFlag, $similarJudulId, $similarJudulText] =
            $this->hitungSimilarity($db, $judulNormalized);

        if ($similarityScore >= 80 && $similarJudulText !== '') {
            return redirect()->back()
                ->withInput()
                ->with(
                    'error',
                    'Judul terlalu mirip dengan judul yang sudah ada. Kemiripan: '
                    . $similarityScore . '%. Judul pembanding: ' . $similarJudulText
                );
        }
        ...
        $pengajuanId = (int) $db->insertID();

        if ($similarityScore >= 60 && $similarJudulText !== '') {
            $db->table('judul_similarity_logs')->insert([
                'pengajuan_judul_id'     => $pengajuanId,
                'compared_with_judul_id' => $similarJudulId > 0 ? $similarJudulId : null,
                'judul_pembanding'       => $similarJudulText,
                'score'                  => $similarityScore,
                'hasil'                  => $similarityScore >= 80 ? 'blocked' : 'warning',
                'created_at'             => date('Y-m-d H:i:s'),
            ]);
        }
        ...
        if (! $db->transStatus()) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Gagal menyimpan pengajuan judul.');
        }

        $message = $similarityScore >= 60
            ? 'Judul berhasil diajukan, namun terdeteksi mirip ' . $similarityScore . '%. Silakan tunggu review pembimbing.'
            : 'Judul berhasil diajukan.';

        return redirect()->to('/pengajuan-judul')->with('success', $message);
    }
    ...
    public function simpanRevisi(int $id)
    {
        ...
        [$similarityScore, $similarityFlag, $similarJudulId, $similarJudulText] =
            $this->hitungSimilarity($db, $judulNormalized, (int) $pengajuanLama['id']);
        ...
        if ($similarityScore >= 80 && $similarJudulText !== '') {
            return redirect()->back()
                ...
                ->with(
                    'error',
                    'Judul revisi terlalu mirip dengan judul yang sudah ada. Kemiripan: '
                    . $similarityScore . '%. Judul pembanding: ' . $similarJudulText
                );
        }
        ...
        $pengajuanBaruId = (int) $db->insertID();
        ...
        if (! $db->transStatus()) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Gagal menyimpan revisi judul.');
        }
    ...
    public function detailMahasiswa(int $id)
    {
        ...
        $similarityLogs = $db->table('judul_similarity_logs')
    ...
    private function normalizeJudul(string $judul): string
    {
        ...
        $judul = preg_replace('/\s+/', ' ', (string) $judul);
        ...
        $words = explode(' ', trim((string) $judul));
    ...
    {
        $builder = $db->table('pengajuan_judul')
            ->select('id, judul, judul_normalized')
            ->whereIn('status', ['diajukan', 'direview', 'disetujui', 'revisi']);
        ...
        foreach ($cekMirip as $item) {
            $existingNormalized = (string) ($item['judul_normalized'] ?? '');

            if ($existingNormalized === '') {
                $existingNormalized = $this->normalizeJudul((string) ($item['judul'] ?? ''));
            }
            ...
            similar_text($judulNormalized, $existingNormalized, $persen);

            $score = round((float) $persen, 2);

            if ($score > $highestScore) {
                $highestScore = $score;
                $similarJudulId = (int) ($item['id'] ?? 0);
                $similarJudulText = (string) ($item['judul'] ?? '');
            }
        }
        ...
        return [$highestScore, $similarityFlag, $similarJudulId, $similarJudulText];
    }
public function cekSimilarity()
{
    $userId = (int) session()->get('user_id');
    ...
    [$score, $flag, $similarId, $similarText] = $this->hitungSimilarity($db, $judulNormalized);

    $status = 'aman';
    $message = 'Judul terlihat aman dan belum terdeteksi mirip signifikan.';

    if ($score >= 80) {
        $status = 'blocked';
        $message = 'Judul terlalu mirip dengan judul yang sudah ada.';
    } elseif ($score >= 60) {
        $status = 'warning';
        $message = 'Judul cukup mirip. Masih bisa diajukan, tetapi perlu diperhatikan.';
    }
