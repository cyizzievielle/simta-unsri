...
<?php
    ...
    $buildPageUrl = static function (int $targetPage) use ($queryParams) {
        ...
        return base_url('/dosen/proposal-ta/riwayat?' . http_build_query($params));
    };
?>

<div class="page-hero">
    <h2>Riwayat Review Proposal</h2>
    <p>Lihat seluruh keputusan final review proposal yang pernah Anda berikan dengan pencarian, filter, dan pagination yang lebih rapi.</p>
</div>
...
<div class="card-premium">
    <h3 class="section-title-premium">Riwayat Review Proposal</h3>
    <p class="section-subtitle-premium">Cari berdasarkan mahasiswa, NIM, judul, atau nama file proposal, lalu atur jumlah data per halaman.</p>
        ...
        <form method="get" action="<?= base_url('/dosen/proposal-ta/riwayat') ?>">
            <div class="filter-grid">
                <div class="form-group-premium">
                    ...
                    <input type="text" id="q" name="q" value="<?= esc((string) $keyword) ?>" placeholder="Cari data review proposal...">
                </div>
                ...
                <div class="filter-actions">
                    ...
                    <a href="<?= base_url('/dosen/proposal-ta/riwayat') ?>" class="btn" style="border:1px solid #cbd5e1; color:#334155;">Reset</a>
                </div>
            </div>
        </form>
    </div>
    ...
    <div style="display:flex; gap:8px; flex-wrap:wrap;">
        <a href="<?= base_url('/dosen/proposal-ta/review/edit/' . (string) ($row['id'] ?? '')) ?>"
           class="btn btn-primary" style="padding:8px 12px; font-size:13px;">
            ✎ Edit
        </a>

        <form action="<?= base_url('/dosen/proposal-ta/review/delete/' . (string) ($row['id'] ?? '')) ?>" method="post" onsubmit="return confirm('Hapus review ini?');" style="display:inline;">
            <?= csrf_field() ?>
    ...
    <?php else: ?>
        <div class="premium-empty">Belum ada riwayat review proposal yang cocok dengan filter.</div>
    <?php endif; ?>
